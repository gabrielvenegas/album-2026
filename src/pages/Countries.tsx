...
const CONF_ORDER: Confederation[] = ['CONMEBOL', 'UEFA', 'CONCACAF', 'AFC', 'CAF', 'OFC']
...
export function Countries() {
  ...
  return (
    <div className="scroll-area flex-1">
      <div className="sticky top-0 z-10 bg-bg px-4 pt-6 pb-3 space-y-3">
      ...
      <div className="px-4 pb-6 space-y-6">
        {view === 'group' ? (
          ...
          CONF_ORDER.map(conf => {
            const countries = filtered.filter(c => c.confederation === conf)
            if (!countries.length) return null
            return (
              <div key={conf}>
                <h2 className="text-xs font-semibold text-muted uppercase tracking-widest mb-3">
