    ...
    <div className="flex flex-col flex-1 min-h-0">
      <div className="flex-none px-4 pt-4 pb-3 bg-bg border-b border-border">
        <div className="flex items-center gap-3 mb-3">
          <button onClick={() => navigate('/paises')} className="text-muted p-1 -ml-1">
            <ArrowLeft size={20} />
          </button>
          <span className="text-3xl">{country.flag}</span>
          <div className="flex-1">
            <h1 className="text-lg font-bold text-text">{country.name}</h1>
            <p className="text-xs text-muted">{country.code} · {pct}% completo</p>
          </div>
        </div>
        ...
        <div className="flex justify-between text-[10px] text-muted">
          <span>{owned}/{total} coletadas</span>
      ...
      <div className="scroll-area flex-1 px-3 py-3">
        ...
        <div className="flex gap-4 justify-center text-[10px] text-muted mb-2">
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-sm bg-missing inline-block" /> Faltando
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-sm bg-owned inline-block" /> Coletada
          </span>
          <span className="flex items-center gap-1">
            <span className="w-2 h-2 rounded-sm bg-duplicate inline-block" /> Repetida
          </span>
        </div>
        ...
        <div className="flex gap-2">
          ...
          {!confirmReset ? (
            ...
            <button
              onClick={() => { resetCountry(country.code, codes); setConfirmReset(false) }}
