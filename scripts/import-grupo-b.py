#!/usr/bin/env python3
"""Recorta figurinhas do PDF Grupo B (grid 4x4) e salva em public/stickers/."""
...
PDF = Path("/Users/marllondiniz/Desktop/GRUPO B.pdf")
...
def _mex_grid(
    ...
    """Permutação de células igual à página do México (Grupo A)."""
    return {
        (page, 0, 0): 1,
        (page, 0, 1): 13,
        (page, 0, 2): 2,
        ...
        (page, 3, 2): 17,
        (page, 3, 3): 16,
        (page5, page5_row, 0): 15,
        (page5, page5_row, 1): 18,
        (page5, page5_row, 2): 20,
        (page5, page5_row, 3): 19,
    }


# Ordem das células (page, row, col) -> número da figurinha no álbum.
# Pág. 1–4: grade 4x4 (16 figurinhas). Pág. 5: uma linha por país (slots finais).
SLOTS: dict[str, dict[tuple[int, int, int], int]] = {
    # Canadá — leitura em ordem no PDF (escudo, foto, jogadores 2→16)
    "can": {
        (1, 0, 0): 1,
        (1, 0, 1): 13,
        (1, 0, 2): 2,
    ...
    # Bósnia, Qatar, Suíça — mesma permutação de células do México (Grupo A pág. 1)
    "bih": _mex_grid(2, 5, 1),
    "qat": _mex_grid(3, 5, 2),
    "sui": _mex_grid(4, 5, 3),
}
...
def main() -> None:
    ...
    manifest = ROOT / "src" / "data" / "grupo-b-images.json"
